...
class FunctionLogger:
    ...
    def __init__(
        ...
        if self.noise_flag:
            ...
            self._S = np.full([cache_size, 1], np.nan)
    ...
    def __call__(self, x: np.ndarray):
        ...
        try:
            ...
            if self.noise_flag and self.uncertainty_handling_level == 2:
                ...
                log_like_val, log_prior_val_orig, fsd = self.fun(x_orig)
            else:
    ...
    def _expand_arrays(self, resize_amount: int = None):
        ...
        if self.noise_flag:
            self._S = np.append(
                self._S, np.full([resize_amount, 1], np.nan), axis=0
            )
            self.S = np.append(
    ...
    def _record(
        ...
        if np.any(duplicate_flag):
            ...
            N = self.nevals[idx]
            if fsd is not None:
                tau_n = 1 / self._S[idx] ** 2
                tau_1 = 1 / fsd**2
                self.y_orig[idx] = (
                    tau_n * self.y_orig[idx] + tau_1 * fval_orig
                ) / (tau_n + tau_1)
                self._S[idx] = 1 / np.sqrt(tau_n + tau_1)
                self.S[idx] = self.beta * self._S[idx]
