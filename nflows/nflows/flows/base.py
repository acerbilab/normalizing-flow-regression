...
class Flow(Distribution):
    ...
    def _log_prob(self, inputs, context, return_aux=False):
        embedded_context = self._embedding_net(context)
        res = self._transform(inputs, context=embedded_context)
        if len(res) == 3:
            noise, logabsdet, aux = res
        else:
            noise, logabsdet = res
            aux = None
    ...
    def _sample(self, num_samples, context):
        ...
        res = self._transform.inverse(noise, context=embedded_context)
        if len(res) == 3:
            samples, _, _ = res
        else:
            samples, _ = res
    ...
    def transform_to_noise(self, inputs, context=None):
        ...
        """
        res = self._transform(inputs, context=self._embedding_net(context))
        if len(res) == 3:
            noise, _, _ = res
        else:
            noise, _ = res
