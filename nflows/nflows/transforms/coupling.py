...
class CouplingTransform(Transform):
    ...
    def __init__(
        self, mask, transform_net_create_fn, unconditional_transform=None
    ):
        """
        Constructor.
        ...
        assert (
            self.num_identity_features + self.num_transform_features
            == self.features
        )
    ...
    def _coupling_transform_forward(self, inputs, transform_params):
        if len(inputs.shape) == 2:
            ...
            z, jac = self.transformer(
                ...
                transform_params.permute(0, 2, 3, 1).reshape(
                    -1, 1, transform_params.shape[1]
                ),
            )
...
class AffineCouplingTransform(CouplingTransform):
    ...
    def __init__(
        ...
        super().__init__(
            mask, transform_net_create_fn, unconditional_transform
        )
...
class PiecewiseCouplingTransform(CouplingTransform):
    def _coupling_transform_forward(self, inputs, transform_params):
        return self._coupling_transform(
            inputs, transform_params, inverse=False
        )
...
class PiecewiseCubicCouplingTransform(PiecewiseCouplingTransform):
    ...
    def _piecewise_cdf(self, inputs, transform_params, inverse=False):
        ...
        unnorm_derivatives_left = transform_params[..., 2 * self.num_bins][
            ..., None
        ]
        unnorm_derivatives_right = transform_params[
            ..., 2 * self.num_bins + 1
        ][..., None]
    ...
    def __init__(
        ...
        if apply_unconditional_transform:
            unconditional_transform = (
                lambda features: PiecewiseRationalQuadraticCDF(
                    shape=[features] + (img_shape if img_shape else []),
                    num_bins=num_bins,
                    tails=tails,
                    tail_bound=tail_bound,
