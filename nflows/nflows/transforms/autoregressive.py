...
class AutoregressiveTransform(Transform):
    ...
    def forward(self, inputs, context=None):
        ...
        res = self._elementwise_forward(inputs, autoregressive_params)
        if len(res) == 2:
            outputs, logabsdet = res
            aux = None
        elif len(res) == 3:
            outputs, logabsdet, aux = res
        else:
            raise ValueError()
        return outputs, logabsdet, aux
    ...
    def inverse(self, inputs, context=None):
        ...
        for _ in range(num_inputs):
            ...
            res = self._elementwise_inverse(inputs, autoregressive_params)
            if len(res) == 3:
                outputs, logabsdet, aux = res
            else:
                outputs, logabsdet = res
...
class MaskedAffineAutoregressiveTransform(AutoregressiveTransform):
    def __init__(
        ...
        constrain_transform_ranges=False,
        transform_range_scales=None,
        return_aux=False,
    ):
        ...
        self._epsilon = 1e-3
        self.constrain_transform_ranges = constrain_transform_ranges
        self.transform_range_scales = transform_range_scales
    ...
    def _elementwise_forward(self, inputs, autoregressive_params):
        ...
        scale, shift, aux = self._scale_and_shift(
            autoregressive_params, inputs
        )
        log_scale = torch.log(scale)
        outputs = scale * inputs + shift
        logabsdet = torchutils.sum_except_batch(log_scale, num_batch_dims=1)
        aux.update({"scale": scale, "shift": shift})
        # print(
        #     f"scale (min, max): ({scale.min().item():.2}, {scale.max().item():.2}) ; max |shift|: {shift.abs().max().item():.2}"
        # )
        if self.return_aux:
            return outputs, logabsdet, aux
    ...
    def _scale_and_shift(self, autoregressive_params, inputs):
        raw_alpha, raw_mu = self._unconstrained_scale_and_shift(
            autoregressive_params
        )
        if self.return_aux:
            aux = {"raw_scale": raw_alpha, "raw_shift": raw_mu}
        else:
            aux = {}

        if self.constrain_transform_ranges:
            inscale_alpha, inscale_mu = (
                self.transform_range_scales["inscale_alpha"],
                self.transform_range_scales["inscale_mu"],
            )
            outcale_alpha, outscale_mu = (
                self.transform_range_scales["outcale_alpha"],
                self.transform_range_scales["outscale_mu"],
            )
            scale = torch.exp(
                np.log(outcale_alpha) * torch.tanh(raw_alpha / inscale_alpha)
            )
            shift = outscale_mu * torch.tanh(raw_mu / inscale_mu)
        else:
            scale = F.softplus(raw_alpha) + self._epsilon
            shift = raw_mu

        return scale, shift, aux
